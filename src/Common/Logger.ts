/**
 * Very basic logger, use booleans to turn specific levels of logger on and off
...
export class Logger {
    ...
    public static COLOR_VERBOSE = new Vector3D(128, 128, 128);
    public static COLOR_DEBUG = new Vector3D(255, 255, 255);
    public static COLOR_WARNING = new Vector3D(255, 255, 0);
    public static COLOR_CRITICAL = new Vector3D(255, 0, 0);

    public static LogVerbose(...params: any[]) {
        if (Logger.doLogVerbose) {
            print(Vector3D.textString(Logger.COLOR_VERBOSE, "Verbose: ", ...params));
        }
    }

    public static LogVerboseSolo(params: any) {
        if (Logger.doLogVerbose) {
            print(Vector3D.textStringSolo(Logger.COLOR_VERBOSE, "Debug: " + params));
        }
    }

    public static LogDebug(...params: any[]) {
        if (Logger.doLogDebug) {
            print(Vector3D.textString(Logger.COLOR_DEBUG, "Debug: ", ...params));
        }
    }

    public static LogDebugSolo(params: any) {
        if (Logger.doLogDebug) {
            print(Vector3D.textStringSolo(Logger.COLOR_DEBUG, "Debug: " + params));
        }
    }

    public static LogWarning(...params: any[]) {
        if (Logger.doLogWarning) {
            print(Vector3D.textString(Logger.COLOR_WARNING, "Warning: ", ...params));
        }
    }

    public static LogWarningSolo(params: any) {
        if (Logger.doLogWarning) {
            print(Vector3D.textStringSolo(Logger.COLOR_WARNING, "Warning: " + params));
        }
    }

    public static LogCritical(...params: any[]) {
        if (Logger.doLogCritical) {
            print(Vector3D.textString(Logger.COLOR_CRITICAL, "Critical: ", ...params));
        }
    }

    public static LogCriticalSolo(param: any) {
        if (Logger.doLogCritical) {
            print(Vector3D.textStringSolo(Logger.COLOR_CRITICAL, "Critical: " + param));
        }
    }
