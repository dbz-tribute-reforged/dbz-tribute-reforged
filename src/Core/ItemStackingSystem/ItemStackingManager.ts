...
export class ItemStackingManager {
  static readonly ITEM_PICKUP_RANGE: number = 300;
  static readonly MAX_PICKUP_TIMERS: number = 50;
  static readonly DELAYED_PICKUP_CHECK_INTERVAL: number = 0.03;
  static readonly DELAYED_PICKUP_CHECK_DURATION: number = 1000;
  ...
  unitNearItem(pickupUnit: unit, pickupItem: item): boolean {
    ...
    const result: boolean = DistanceBetweenPoints(unitLoc, itemLoc) < ItemStackingManager.ITEM_PICKUP_RANGE;
  ...
  setupDelayedPickup(
    ...
    if (this.numDelayedPickupTimer < ItemStackingManager.MAX_PICKUP_TIMERS) {
      ...
      TimerStart(delayedPickupTimer, ItemStackingManager.DELAYED_PICKUP_CHECK_INTERVAL, true, () => {
        if (timerLife > ItemStackingManager.DELAYED_PICKUP_CHECK_DURATION) {
          --this.numDelayedPickupTimer;
